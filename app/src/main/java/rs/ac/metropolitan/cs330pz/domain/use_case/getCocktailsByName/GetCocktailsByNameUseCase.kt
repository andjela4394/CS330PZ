...
class GetCocktailsByNameUseCase @Inject constructor(private val repository : CocktailRepository) {

    operator fun invoke(cocktailName: String): Flow<Resource<List<Cocktail>>> = flow {
        try {
            emit(Resource.Loading<List<Cocktail>>())
            val cocktails = repository.getCocktailsByName(cocktailName).drinks.map { it.toCocktail() }
