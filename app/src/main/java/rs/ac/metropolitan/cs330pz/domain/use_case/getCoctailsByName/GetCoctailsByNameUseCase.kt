package rs.ac.metropolitan.cs330pz.domain.use_case.getCoctailsByName
...
class GetCoctailsByNameUseCase @Inject constructor(private val repository : CocktailRepository) {

    operator fun invoke(cocktailName: String): Flow<Resource<List<Cocktail>>> = flow {
        try {
            emit(Resource.Loading<List<Cocktail>>())
            val cocktails = repository.getCocktailsByName(cocktailName).map { it.toCocktail() }
